...
namespace Geodesy
{
    ...
    public struct UtmGrid
    {
        ...
        public char Band
        {
            ...
            set
            {
                BandNr = BandChars.IndexOf(value);
            }
        }
        ...
        {
            if (null==projection)
                throw new ArgumentNullException(Properties.Resources.PROJECTION_NULL);
        ...
        public UtmGrid(UtmProjection projection, GlobalCoordinates coord) : this(projection)
        {
            ...
            if (_zone == 31 && Band == 'V')
            {
                double delta = coord.Longitude.Degrees - _llCoordinates.Longitude.Degrees - Width.Degrees;
            ...
            else if (Band == 'X')
            {
                ...
                {
                    double delta = coord.Longitude.Degrees - CenterMeridian.Degrees;
                    if (Math.Sign(delta)==-1)
                        Zone = _zone - 1;
                    else
                        Zone = _zone + 1;
